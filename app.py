"""Main ETQX API app combining strategy endpoints and capabilities/protocols APIs."""
import os, json, math
import logging
...
SCHEMA = {
    "actors": {
        ...
        "powerAsymmetry": "you|other|balanced",
    },
...
def base_map(f: dict):
    ...
    horizon = {"short": 0, "medium": 0.5, "long": 1}.get(
        f.get("timeHorizon", "medium"), 0.5
    )
    ...
    rationale = [
        f"γ from trust/commitments/horizon; θ/φ from risk & uncertainty; asym={asym}."
    ]
...
def apply_strategy(strategy: str, f: dict):
    ...
    if strategy == "cooperative_alignment":
        ...
        payoff = {
            "r": 3.0 + 0.4 * trust,
            "p": 1.0 + 0.2 * to01(f.get("uncertainty", "medium")),
            "t": 5.0 - 0.3 * trust,
            "s": 0.0 + 0.2 * (1 - trust),
        }
        if trust >= 0.5:
            ...
            rat += [
                "Symmetric Q strategy under strong entanglement for cooperative alignment."
            ]
        else:
            ...
            rat += [
                "Low-trust preset: symmetric exploration with light coupling and modest phase."
            ]
        ...
        rat += [
            "Self-alignment: reward coherence (r↑), penalize drift (p↑), suppress temptations (t/s↓)."
        ]
...
COACH_SCHEMA = {"tips": ["string"], "actions": ["string"], "tuning": ["string"], "warnings": ["string"]}
...
async def coach_tips_llm(
    ...
    brief = {
        ...
        "features": {
            ...
            "powerAsymmetry": (features.get("actors") or {}).get(
                "powerAsymmetry", "balanced"
            ),
        ...
        "quantum": {
            "you": round(quantum["you"], 4),
            "other": round(quantum["other"], 4),
        },
        "classical": {
            "you": round(classical["you"], 4),
            "other": round(classical["other"], 4),
        },
        "metrics": {
            k: (v if not isinstance(v, dict) else {ik: round(iv, 4) for ik, iv in v.items()})
...
# Minimal logging config for standalone mode
_lvl = os.getenv("ETQX_LOG_LEVEL", "INFO").upper()
logging.basicConfig(
...
# -------------------------------------------------
# Mount capabilities and protocols APIs from package utilities
# -------------------------------------------------
...
try:
    _cfg = _load_module("_etqx_cfg", "config.py")
    _fs = _load_module("_etqx_fs", "utils/fs.py")
    _capm = _load_module("_etqx_capm", "models/capability.py")
    _protm = _load_module("_etqx_protm", "models/protocol.py")

    settings = _cfg.settings
    CapabilityItem = _capm.CapabilityItem
    CapabilityDoc = _capm.CapabilityDoc
    ProtocolItem = _protm.ProtocolItem
    ProtocolDoc = _protm.ProtocolDoc
    list_capability_files = _fs.list_capability_files
    slug_from_file = _fs.slug_from_file
    file_for_slug = _fs.file_for_slug
    read_utf8 = _fs.read_utf8
    derive_name = _fs.derive_name
    file_etag = _fs.file_etag
    ensure_dir_exists = _fs.ensure_dir_exists

    # Capabilities router
    cap_router = APIRouter(prefix="/api/capabilities", tags=["capabilities"])

    @cap_router.get("", response_model=dict)
    ...
    def get_capability_endpoint(
        ...
        try:
            ...
            path = file_for_slug(settings.capabilities_dir, cid)
            if not path.exists():
    ...
    # Protocols router
    prot_router = APIRouter(prefix="/api/protocols", tags=["protocols"])

    @prot_router.get("", response_model=dict)
    def list_protocols_endpoint(
        format: str = Query(default="json", pattern="^(json|txt)$")
    ):
        try:
            files = list_capability_files(settings.protocols_dir)
        except FileNotFoundError:
    ...
    def get_protocol_endpoint(
        ...
        try:
            ...
            path = file_for_slug(settings.protocols_dir, pid)
            if not path.exists():
                raise HTTPException(status_code=404, detail="not found")
            text = read_utf8(path)
        except ValueError as e:
    ...
    app.include_router(prot_router)
except Exception as e:
    logging.getLogger("etqx.bootstrap").warning(
        "Capabilities/Protocols bootstrap skipped: %s", e
    )
